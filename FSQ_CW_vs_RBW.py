...
def measure(rbw_vec, exp, station, current_point=0.):
    ...
    meas.register_parameter(FSQ.res_BW)
    meas.register_parameter(FSQ.spectrum,setpoints=[FSQ.res_BW])
    ...
    with meas.run() as datasaver:
        ...
        for idx, f in enumerate(rbw_vec):
            ...
            get_f = int(FSQ.res_BW.get())
            get_s = FSQ.spectrum.get()

            sleep(0.01)

            datasaver.add_result((FSQ.res_BW, get_f),
                                (FSQ.spectrum,get_s))
...
#
################################################################################
FSQ_rbw_vec    = np.array([0.3e3,1e3,3e3,10e3,30e3,100e3,300e3,1e6,3e6,10e6,20e6])
FSQ_rbw_points =  len(FSQ_rbw_vec)
...
FSQ_time_sweep =  float(FSQ.sweep_time_direct())

parameter_snap['FSQ']['sweeptime'] = FSQ_time_sweep

print('\ntime for one trace : {}'.format(datetime.timedelta(seconds=FSQ_time_sweep)))
print('\ntime estimation for all measurement : {} '
      .format(datetime.timedelta(seconds=FSQ_avg*FSQ_rbw_points*FSQ_time_sweep)))
...
measure(FSQ_rbw_vec,exp=exp, station=station,current_point=current)
