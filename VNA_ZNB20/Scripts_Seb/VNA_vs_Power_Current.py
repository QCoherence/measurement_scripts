...
fnames = glob.glob('windows/f_vec_' + '*')
c_vec = np.array([fnames[k].replace('windows\\f_vec_', '') for k in xrange(len(fnames))])
c_vec = np.array([np.float(c_vec[k].replace('.dat', '')) for k in xrange(len(fnames))])

#Detector bandwidth [Hz]
# detector_bandwidth = 2# Hz
detector_bandwidth_vec = np.array([4, 4, 8, 8, 10, 20, 40, 50, 80, 100, 100, 100, 100, 100, 200, 200, 200])
...
Power_stop = 10
Power_step = 2.5
Power_vec = np.arange(Power_start, Power_stop + Power_step, Power_step)
...
#We save data
    qt.mstart()
    filename = 'VNA_vs_Power_Current_'+str(curr_vec[c]*1e3)+'_mA' #Filename that will be used to name the data folder. Use script filename.
    ...
    plot2d_1= qt.Plot2D(data, name='last_trace', coorddim=2, valdim=3, maxtraces=2)
    plot2d_2= qt.Plot2D(data, name='last_trace_phase', coorddim=2, valdim=4, maxtraces=2)
    if ref: 
        plot2d_3= qt.Plot2D(data, name='last_trace_corrected', coorddim=2, valdim=5, maxtraces=2)
        plot2d_4= qt.Plot2D(data, name='last_trace_phase_corrected', coorddim=2, valdim=6, maxtraces=2)

    plot3d_1 = qt.Plot3D(data, name='S21_dB', coorddims=(1,2), valdim=3,style='image')
    plot3d_2 = qt.Plot3D(data, name='S21_phase', coorddims=(1,2), valdim=4,style='image')
    plot3d_1.set_palette('afmhot')
    plot3d_2.set_palette('afmhot')
    if ref:
        plot3d_3 = qt.Plot3D(data, name='S21_dB_normalized', coorddims=(1,2), valdim=5,style='image')
        plot3d_4 = qt.Plot3D(data, name='S21_phase_normalized', coorddims=(1,2), valdim=6,style='image')
    ...
    for p in xrange(len(Power_vec)):
        ...
        try:
            ...
            if ref: 
                ...
                data.add_data_point((curr_vec[c])*np.ones_like(freqs)*1e3,(Power_vec[p])*np.ones_like(freqs),freqs,amp_final,phase_final,amp_corr,phase_corr)
            else: 
                data.add_data_point((curr_vec[c])*np.ones_like(freqs)*1e3,(Power_vec[p])*np.ones_like(freqs),freqs,amp_final,phase_final)
            data.new_block()
