...
def get_count_states(x_g, x_e, threshold):
    ...
    if e_morethan_g(p_left_g, p_left_e, p_right_g, p_right_e):
        ...
        p_gg = p_left_g
        p_ee = p_right_e
        p_ge = p_right_g  # P to measure e when g was prepared
        p_eg = p_left_e # P to measure g when e was prepared
    else:
        sign = -1
        p_gg = p_right_g
        p_ee = p_left_e
        p_ge = p_left_g # P to measure e when g was prepared
        p_eg = p_right_e # P to measure g when e was prepared
...
def get_correlations(x1_g, x1_e, x2_g, x2_e, threshold, sign_ge):
    '''
    Function to count number of values according to threshold
    Returns dictionary
    p_gg
    p_ee
    p_ge = P to measure g on 1st readout and e on 2nd readout
    p_eg
    '''

    def g_state(val, threshold, sign_ge):
        '''
        small function takes value on x-axes (normalized), threshold value and sign
        return true if value is belongs to |g>-state and False if belongs to |e>.
        '''
        if sign_ge > 0:
            if val > threshold:
                return False
    ...
    if len(x1_g)<1 or len(x1_e)<1 or len(x2_g)<1 or len(x2_e)<1:
        print 'error get_count_states. No data'
        return None
    ...
    # Statistics for the qubit prepared in g-state
    for i in range(len(x1_g)):
        if g_state(x1_g[i], threshold, sign_ge) and g_state(x2_g[i], threshold, sign_ge):
            n_1g_2g += 1
        if not g_state(x1_g[i], threshold, sign_ge) and not g_state(x2_g[i], threshold, sign_ge):
            n_1e_2e += 1
        if g_state(x1_g[i], threshold, sign_ge) and not g_state(x2_g[i], threshold, sign_ge):
            n_1g_2e += 1
        if not g_state(x1_g[i], threshold, sign_ge) and g_state(x2_g[i], threshold, sign_ge):
            n_1e_2g += 1

    # Statistics for the qubit prepared in e-state
    for i in range(len(x1_e)):
        if g_state(x1_e[i], threshold, sign_ge) and g_state(x2_e[i], threshold, sign_ge):
            n_1g_2g += 1
        if not g_state(x1_e[i], threshold, sign_ge) and not g_state(x2_e[i], threshold, sign_ge):
            n_1e_2e += 1
        if g_state(x1_e[i], threshold, sign_ge) and not g_state(x2_e[i], threshold, sign_ge):
            n_1g_2e += 1
        if not g_state(x1_e[i], threshold, sign_ge) and g_state(x2_e[i], threshold, sign_ge):
            n_1e_2g += 1

    #probabilities to get same or different results on 1st and 2nd readout
    p_gg = 1.0*n_1g_2g / len(x1_g)
    p_ee = 1.0*n_1e_2e / len(x1_e)
    p_ge = 1.0*n_1g_2e / len(x1_g)
    p_eg = 1.0*n_1e_2g / len(x1_e)
...
def add_SecondGaussian_fit(Hist_x_g, Hist_x_e, PLOT = False):
    ...
    def fit_gauss(x, y, gauss_par0, fixed=[0,2,3], crop_window=[None,None]):
        ...
        gauss_par0 = [0-expec_min_y, 1-expec_max_y, 2-expec_center_x, 3-expec_std_x]  ## (!) max_y is not y maximum, it is area
    ...
    def convert_parameters_as_in_gauss_param(gauss_p):
        min_y       = gauss_p[0]
        max_y       = gauss_p[1]
        center_v    = gauss_p[2]
        fw_at_06    = gauss_p[3]
        sigma_v  = fw_at_06/2  ## HERE IS CHANGE
        p_list      = [ center_v, sigma_v, fw_at_06, max_y, min_y ]
        return p_list

    p_list_ge = convert_parameters_as_in_gauss_param(gauss_p_ge)
    p_list_eg = convert_parameters_as_in_gauss_param(gauss_p_eg)

    ### to save
    # Hist_x_e.gauss2_param = gauss_p_eg
    Hist_x_e.gauss2_param = p_list_eg
    Hist_x_e.gauss2_fit   = [y_gausfit_eg, X]

    # Hist_x_g.gauss2_param = gauss_p_ge
    Hist_x_g.gauss2_param = p_list_ge
...
class Histogram:
    ...
    def fit(self, threshold, crop_sign, do_crop=True):
        ...
        def fit_gauss(hist_x_xy, crop_thr=None, crop_sign = +1):
            ...
            p_list      = [ center_v, sigma_v, fw_at_06, max_y, min_y ] ## max_y is not y maximum, it is area
...
class SSResult:
    ...
    def __init__(self, data=None, datafile=None, paramfile=None, param=None, nbins=None, all_included=True, thr_set_zero=True, shift_to_center=False):
        ...
        if self.datafile != None:
            print "__autosaved to .pkl file"
            self.save_ssr_as_object(self.datafile[:-4])
        else:
            print "Can not autosave ssr - self.datafile is None"
    ...
    def make_postselected_data_from_norm(self):
        ...
        ## Selected data gonna be there
        x_g_selected = np.array([])
        x_e_selected = np.array([])
        y_g_selected = np.array([])
        y_e_selected = np.array([])

        ## Save also data wich we discard (if you need to analize it as well)
        x_g_discarded = np.array([])
        x_e_discarded = np.array([])
        y_g_discarded = np.array([])
        y_e_discarded = np.array([])
        ...
        ## Working on data with prepared |g>
        for i in range(len(self.x_g)):
            if g_state(self.x_g_pre[i], self.threshold, self.sign_ge):
                ...
                x_g_discarded = np.append(x_g_discarded, self.x_g[i])
                y_g_discarded = np.append(y_g_discarded, self.y_g[i])

        ## Working on data with prepared |e>
        for i in range(len(self.x_e)):
            if g_state(self.x_e_pre[i], self.threshold, self.sign_ge):
                ...
                x_e_discarded = np.append(x_e_discarded, self.x_e[i])
                y_e_discarded = np.append(y_e_discarded, self.y_e[i])
        ...
        self.x_g_discarded = x_g_discarded
        self.x_e_discarded = x_e_discarded
        self.y_g_discarded = y_g_discarded
        self.y_e_discarded = y_e_discarded

        print '-- before selection --', len(self.x_g)
        print '-- selected --', len(self.x_g_select)
        discarded = len(self.x_g) - len(self.x_g_select)
        print '-- discarded [%] --', 100.0*( 1.0*discarded / len(self.x_g) )
    ...
    def make_histograms(self, nbins=None):
        ...
        if self.hist_x_g_select is not None and self.hist_x_e_select is not None:
            [self.hist_x_g_select, self.hist_x_e_select] = add_SecondGaussian_fit(self.hist_x_g_select, self.hist_x_e_select)
    ...
    def calculate_fidelity_post(self):
        ...
        ### COMPARE AREAS UNDER FIT CURVES FOR MAIN STATE AND SIDE #####
        main_g_area = self.hist_x_g_select.gauss_param[3]
        main_e_area = self.hist_x_e_select.gauss_param[3]
        side_g_area = self.hist_x_g_select.gauss2_param[3]
        side_e_area = self.hist_x_e_select.gauss2_param[3]
        P_fit_select_decay  = side_e_area / main_e_area
        P_fit_select_excite = side_g_area / main_g_area
        ...
        self.dict_fidelity['P_fit_select_decay']          = P_fit_select_decay
        self.dict_fidelity['P_fit_select_excite']         = P_fit_select_excite



        self.distance = abs( self.center_x_g_select - self.center_x_e_select )
        self.sizemean = np.mean( [ self.sizeblob_x_g, self.sizeblob_x_e ] )  #carefoul - it's not from selected. a bit less precise
        self.snr_dB = 20 *np.log10( self.distance / self.sizemean )  ##= 20*log_10
        ...
        if regime == 'raw_data':
            ...
            if (self.hist_x_g_select is not None) and (self.hist_x_e_select is not None):
                ...
                if SHOW_FIT:
                    ...
                    if (self.hist_x_g_select.gauss2_fit is not None) and (self.hist_x_e_select.gauss2_fit is not None):
                        hist_g  = self.hist_x_g_select.gauss2_fit
                        hist_e  = self.hist_x_e_select.gauss2_fit
                        # maxval = np.max([ maxval, np.max(hist_g_fit[0]), np.max(hist_e_fit[0]) ])
                        plt.plot(hist_g[1], hist_g[0], lw=cs['hist_fit_lw'], color=cs['hist_fit_g_color'], label='Fit g-state second selected', alpha=cs['hist_fit_single_alpha'])
                        plt.plot(hist_e[1], hist_e[0], lw=cs['hist_fit_lw'], color=cs['hist_fit_e_color'], label='Fit e-state second selected', alpha=cs['hist_fit_single_alpha'])
    ...
    def save_ssr_as_object(self, filename, protocol=2):
        '''
        Saving all data using pickle.
        '''
        filename += '.pkl'

        if protocol is None:
            protocol = pickle.HIGHEST_PROTOCOL  ## important to use same protocol on all the machines where open or save

        with open(filename, 'wb') as output:
            pickle.dump(self, output, protocol)

        return True

    def autosafe(self, protocol=2):
        try:
            filename = self.datafile[:-4]
            self.save_ssr_as_object(filename)
            return True
        except:
            print("Cant save in pkl using self.datafile")
            return False


class QNDnessResult:
    '''
    This class is linked with one QNDness measurement.
    Data takes only from file during inizialization.
    Contains re and im parts of measured g and e states for pre-pulse, 1st readout and 2nd readout.
    '''
    ###---------------------------###
    ###### DATA ######
    ### Raw Data ###
    ...
    hist_x1_g         = None
    hist_x1_e         = None
    hist_x1_g_select  = None
    hist_x1_e_select  = None

    hist_x2_g         = None
    hist_x2_e         = None
    hist_x2_g_select  = None
    hist_x2_e_select  = None
    ...
    hist_y1_g         = None
    hist_y1_e         = None
    hist_y1_g_select  = None
    hist_y1_e_select  = None

    hist_y2_g         = None
    hist_y2_e         = None
    hist_y2_g_select  = None
    hist_y2_e_select  = None
    ...
    center_y2_g = 0
    center_y2_e = 0
    center_x1_g_select = None # it is different. I dont know why yet
    center_x1_e_select = None
    center_x2_g_select = None # it is different. I dont know why yet
    center_x2_e_select = None
    center_y1_g_select = None # it is different. I dont know why yet
    center_y1_e_select = None
    center_y2_g_select = None # it is different. I dont know why yet
    center_y2_e_select = None
    sizeblob_x1_g = None
    sizeblob_x1_e = None
    sizeblob_x2_g = None
    sizeblob_x2_e = None
    ...
    sizeblob_y2_g = None
    sizeblob_y2_e = None
    squizingblob1_g = None
    squizingblob1_e = None
    squizingblob2_g = None
    squizingblob2_e = None
    ### crosses of size of each blolbs
    ## frame_g_reim = [ax,ay,bx,by,cx,cy,dx,dy]
    cross1_g_xy = None
    cross1_e_xy = None
    ...
    void_r      = None
    threshold_r = None
    center_r1_g_select = None
    center_r1_e_select = None
    center_r2_g_select = None
    center_r2_e_select = None
    ...
    'n_gg'      : 0,    'n_ee'      : 0,
    'n_ge'      : 0,    'n_eg'      : 0,
    'p_gg'      : 0,    'p_ee'      : 0,        # Correlation probabilities between the two readouts
    'p_ge'      : 0,    'p_eg'      : 0,
    }
    ### Count states after postselection ###
    dict_count_ro1_select = {
    'n_left_g'  : 0,    'n_left_e'  : 0,
    'n_right_g' : 0,    'n_right_e' : 0,
    'p_left_g'  : 0,    'p_left_e'  : 0,
    'p_right_g' : 0,    'p_right_e' : 0,
    ...
    def __init__(self, data=None, datafile=None, paramfile=None, param=None, nbins=None, all_included=True, thr_set_zero=True, shift_to_center=False, use_ro2=False):
        ...
        ### Normalised Data after Postselection ###
        self.x1_g_select  = None
        self.x1_e_select  = None
        self.x2_g_select  = None
        self.x2_e_select  = None
        self.y1_g_select  = None
        self.y1_e_select  = None
        self.y2_g_select  = None
        self.y2_e_select  = None
        ...
        self.r2_g         = None
        self.r2_e         = None
        self.r1_g_select  = None
        self.r1_e_select  = None
        self.r2_g_select  = None
        self.r2_e_select  = None

        self.center_r1_g  = None
        self.center_r1_e  = None
        self.center_r2_g  = None
        self.center_r2_e  = None
        self.void_r       = None
        self.void_r       = None
        self.threshold_r  = None
        ...
        self.hist_x2_g         = None
        self.hist_x2_e         = None
        self.hist_x1_g_select  = None
        self.hist_x1_e_select  = None
        self.hist_x2_g_select  = None
        self.hist_x2_e_select  = None
        ...
        self.hist_y2_g         = None
        self.hist_y2_e         = None
        self.hist_y1_g_select  = None
        self.hist_y1_e_select  = None
        self.hist_y2_g_select  = None
        self.hist_y2_e_select  = None

        self.dict_fidelity_ro1 = {
        'F'         : 0,
        'F_g'       : 0,
        'F_e'       : 0,
        'F_post'    : 0,
        ...
        if all_included:
            ...
            try:
                ### shift - make histograms symmetrical around zero
                ###
                self.make_norm_data_from_raw(shift=shift_to_center, use_ro2=use_ro2)
            except:
                print '     ___SSResult error during normalisation'
                self.valid=False
            ...
            ### find the best threshold and shift the data
            try:
                self.set_best_threshold(use_ro2=use_ro2)
                if thr_set_zero:
                    self.shift_x_to_threshold_be_zero(use_ro2=use_ro2)
            except:
                print '     ___SSResult error during setting threshold'
                self.valid=False
            ...
            ### do postselection
            # try:
            if self.make_postselected_data_from_norm(use_ro2=use_ro2) != False:
            #     ### make histograms
            #     ## flag - True\False if fit was succeed
            #     flag1 = self.make_histograms(nbins = nbins)
            #     flag2 = self.make_histograms_y(nbins = nbins)
            ...
            # ### calculate fidelity
            #      self.calculate_fidelity_post()
                self.dict_corr = get_correlations(self.x1_g, self.x1_e, self.x2_g, self.x2_e, self.threshold, self.sign_ge)
                self.dict_corr_select = get_correlations(self.x1_g_select, self.x1_e_select, self.x2_g_select, self.x2_e_select, self.threshold, self.sign_ge)
            else:
                print 'smth went wrong'
            # except:
            #     self.valid=False
            ...
            try:
                ...
                self.valid=False
                print '     ___QNDnessResult error during making unitless histograms'
        ...
        if self.datafile != None:
            print "__autosaved to .pkl file"
            self.save_QNDness_as_object(self.datafile[:-4])
        else:
            print "Can not autosave QNDness - self.datafile is None"
    ...
    def take_data(self, data):
        ...
        try:
            ...
            ### [re_g, im_g, re_e, im_e, re_g_post, im_g_post, re_e_post, im_e_post]
            self.re_g_pre = coef * data[0]
            self.im_g_pre = coef * data[1]
            self.re_e_pre = coef * data[2]
            self.im_e_pre = coef * data[3]
            self.re1_g     = coef * data[4]
            self.im1_g     = coef * data[5]
            self.re1_e     = coef * data[6]
            self.im1_e     = coef * data[7]
            self.re2_g     = coef * data[8]
            self.im2_g     = coef * data[9]
            self.re2_e     = coef * data[10]
            self.im2_e     = coef * data[11]
            return True
    ...
    def load_data(self, datafile):
        ...
        try:
            ...
            raw_data_ss = np.loadtxt(datafile)
            self.re_g_pre = coef * raw_data_ss[:,0]
            self.im_g_pre = coef * raw_data_ss[:,1]
            self.re_e_pre = coef * raw_data_ss[:,2]
            self.im_e_pre = coef * raw_data_ss[:,3]
            self.re1_g     = coef * raw_data_ss[:,4]
            self.im1_g     = coef * raw_data_ss[:,5]
            self.re1_e     = coef * raw_data_ss[:,6]
            self.im1_e     = coef * raw_data_ss[:,7]
            self.re2_g     = coef * raw_data_ss[:,8]
            self.im2_g     = coef * raw_data_ss[:,9]
            self.re2_e     = coef * raw_data_ss[:,10]
            self.im2_e     = coef * raw_data_ss[:,11]
            print 'data loaded'
            return True
    ...
    def make_norm_data_from_raw(self, shift=True, use_ro2=False):
        '''
        Make a normalisation of data on the axis between g_mean and e_mean values using the readout 1 by default
        Write new data as object parameters
        Also make histograms of normalised data
        Returns True if finished
        '''
        if not use_ro2:
            ### check if it is data. if not - try to load from file
            if (self.re1_g is not None) and (self.im1_g is not None) and (self.re1_e is not None) and (self.im1_e is not None):
                re_g = self.re1_g
                im_g = self.im1_g
                re_e = self.re1_e
                im_e = self.im1_e
            ...
            if (self.re2_g is not None) and (self.im2_g is not None) and (self.re2_e is not None) and (self.im2_e is not None):
                re_g = self.re2_g
                im_g = self.im2_g
                re_e = self.re2_e
                im_e = self.im2_e
        ...
        ######____CHANGING_BASIS_____________####################################
        ### change the basis according to positions of blobs centers
        [ [re1_g, im1_g], [re1_e, im1_e] ]                      = change_basis_blobs_inf(threshold_re, threshold_im, theta, [self.re1_g, self.im1_g] , [self.re1_e, self.im1_e] )
        [ [re2_g, im2_g], [re2_e, im2_e] ]                      = change_basis_blobs_inf(threshold_re, threshold_im, theta, [self.re2_g, self.im2_g] , [self.re2_e, self.im2_e] )
        [ [re_g_pre, im_g_pre],  [re_e_pre, im_e_pre] ]     = change_basis_blobs_inf(threshold_re, threshold_im, theta, [re_g_pre, im_g_pre] , [re_e_pre, im_e_pre] )
        ...
        ### new centers:
        [c_x1_g, c_y1_g, c_x1_e, c_y1_e ] = centers_two_blobs(self.x1_g, self.y1_g, self.x1_e, self.y1_e)
        [c_x2_g, c_y2_g, c_x2_e, c_y2_e ] = centers_two_blobs(self.x2_g, self.y2_g, self.x2_e, self.y2_e)

        self.center_x1_g = c_x1_g
        self.center_x1_e = c_x1_e
        self.center_x2_g = c_x2_g
        self.center_x2_e = c_x2_e
        print 'new center of blobs for readout 1: ', self.center_x1_g, ' ', self.center_x1_e
        print 'new center of blobs for readout 2: ', self.center_x2_g, ' ', self.center_x2_e

        print 'data was normalised and saved'
        return True

    def set_best_threshold(self, use_ro2=False):
        '''
        Find best threshold using data of readout 1 by default,
        set it t oobject
        also calculate set self.dict_count values
        returns True if all good
        '''
        ...
        if (self.x1_g is None) or (self.x1_e is None) or (self.x2_g is None) or (self.x2_e is None):
            success_norm = self.make_norm_data_from_raw(use_ro2=use_ro2)
            if not success_norm:
                print 'can not normalise. error of setting threshold'
                return False
        ...
        ### Find best threshold value ###
        if not use_ro2:
            [leftlim, rightlim, rabbish1, rabbish2 ] = crop_fluctuations(self.x1_g, [0] , self.x1_e, [0], 0, 0 )
            delta = abs(rightlim - leftlim)

            threshold = get_best_threshold(self.x1_g, self.x1_e, 0, delta, permiss=1e-4)
        else:
            [leftlim, rightlim, rabbish1, rabbish2 ] = crop_fluctuations(self.x2_g, [0] , self.x2_e, [0], 0, 0 )
            delta = abs(rightlim - leftlim)

            threshold = get_best_threshold(self.x2_g, self.x2_e, 0, delta, permiss=1e-4)

        ##########______SET AND RETURN_____#####################################
        self.threshold = threshold
        self.dict_count_ro1 = get_count_states(self.x1_g,self.x1_e, self.threshold)
        self.dict_count_ro2 = get_count_states(self.x2_g,self.x2_e, self.threshold)
        self.sign1_ge = self.dict_count_ro1['sign_ge']
        self.sign2_ge = self.dict_count_ro2['sign_ge']

        print 'threshold set'
        return True

    def shift_x_to_threshold_be_zero(self, use_ro2=False):
        '''
        Search the best threshold value and shift the data on it
        returns threshold value, that was founded and used for shift
        '''
        if (self.x1_g is None) or (self.x1_e is None) or (self.x2_g is None) or (self.x2_e is None):
            success_norm = self.make_norm_data_from_raw(use_ro2=use_ro2)
            success_th = self.set_best_threshold(use_ro2=use_ro2)      #if we load new norm data - we need to redefine a threshold
            if (not success_norm) or (not success_th):
                print 'can not normalise or set threshold. error of shift_x_to_threshold_be_zero'
                return False

        if self.threshold is None:
            success_th = self.set_best_threshold(use_ro2=use_ro2)
            if not success_th:
                print 'can not set threshold. error of shift_x_to_threshold_be_zero'
                return False
        ...
        self.x1_g = shifter(self.x1_g, threshold)
        self.x1_e = shifter(self.x1_e, threshold)

        self.x2_g = shifter(self.x2_g, threshold)
        self.x2_e = shifter(self.x2_e, threshold)
        ...
        if self.x1_g_select is not None:
            self.x1_g_select = shifter(self.x1_g_select, threshold)
        if self.x2_g_select is not None:
            self.x2_g_select = shifter(self.x2_g_select, threshold)
        if self.x1_e_select is not None:
            self.x1_e_select = shifter(self.x1_e_select, threshold)
        if self.x2_e_select is not None:
            self.x2_e_select = shifter(self.x2_e_select, threshold)
        ...
        ### ( !V uncommented 191110 works same)
        # ### shift the center of blobs also
        if self.center_x1_g is not None:
            [self.center_x1_g] = shifter([self.center_x1_g], threshold)
        if self.center_x2_g is not None:
            [self.center_x2_g] = shifter([self.center_x2_g], threshold)
        if self.center_x1_e is not None:
            [self.center_x1_e] = shifter([self.center_x1_e], threshold)
        if self.center_x2_e is not None:
            [self.center_x2_e] = shifter([self.center_x2_e], threshold)
        print 'new centers of blobs for readout 1:', self.center_x1_g, self.center_x1_e
        print 'new centers of blobs for readout 2:', self.center_x2_g, self.center_x2_e

        ### Kostil: And if the data is shifted we need also to shift histograms if it is exist!
        if (self.hist_x1_g is not None) or (self.hist_x1_e is not None) or (self.hist_x1_g_select is not None)or (self.hist_x1_e_select is not None):
            print 'we redo histograms of readout 1, because of the shift'
            self.make_histograms(readout_nb=1)
        if (self.hist_x2_g is not None) or (self.hist_x2_e is not None) or (self.hist_x2_g_select is not None)or (self.hist_x2_e_select is not None):
            print 'we redo histograms of readout 2, because of the shift'
            self.make_histograms(readout_nb=2)
    ...
    def make_x_dimensionless(self):
        ...
        https://arxiv.org/abs/1506.08165
        '''
        if self.center_x1_g is not None and self.center_x1_e is not None and self.center_x2_g is not None and self.center_x2_e is not None:
            c1_g = self.center_x1_g
            c1_e = self.center_x1_e
            c2_g = self.center_x2_g
            c2_e = self.center_x2_e
        else:
            print 'cant make dimensionless without centers'
        ...
        self.r_g_pre    = (self.x_g_pre - shift ) * coef
        self.r_e_pre    = (self.x_e_pre - shift ) * coef
        self.r1_g        = (self.x1_g - shift ) * coef
        self.r1_e        = (self.x1_e - shift ) * coef
        self.r2_g        = (self.x2_g - shift ) * coef
        self.r2_e        = (self.x2_e - shift ) * coef

        self.r1_g_select = (self.x1_g_select - shift ) * coef
        self.r1_e_select = (self.x1_e_select - shift ) * coef
        self.r2_g_select = (self.x2_g_select - shift ) * coef
        self.r2_e_select = (self.x2_e_select - shift ) * coef

        self.center_r1_g = (self.center_x1_g - shift ) * coef
        self.center_r1_e = (self.center_x1_e - shift ) * coef
        self.center_r2_g = (self.center_x2_g - shift ) * coef
        self.center_r2_e = (self.center_x2_e - shift ) * coef

        self.center_r1_g_select = (self.center_x1_g_select - shift ) * coef
        self.center_r1_e_select = (self.center_x1_e_select - shift ) * coef
        self.center_r2_g_select = (self.center_x2_g_select - shift ) * coef
        self.center_r2_e_select = (self.center_x2_e_select - shift ) * coef
        self.void_r     = (self.void_x - shift ) * coef
        self.threshold_r= (self.threshold - shift ) * coef

        return

    def make_postselected_data_from_norm(self, use_ro2=False):
        '''
        Do the postselection. Threshold usually=0 because we did set_best_threshold_as_zero() before
        also calculate and set dict_count_select
        '''
        ...
        if (self.x1_g is None) or (self.x1_e is None) or (self.x2_g is None) or (self.x2_e is None) or (self.x_g_pre is None) or (self.x_e_pre is None):
            success_norm = self.make_norm_data_from_raw(use_ro2=use_ro2)
            success_th = self.set_best_threshold(use_ro2=use_ro2)      #if we load new norm data - we need to redefine a threshold
            if (not success_norm) or (not success_th):
                print 'can not Postselect. error of normalisation or setting threshold'
        ...
        if len(self.x1_g)!=len(self.x_g_pre) or len(self.x1_e)!=len(self.x_e_pre) or len(self.x2_g)!=len(self.x_g_pre) or len(self.x2_e)!=len(self.x_e_pre):
            print 'ERROR make_postselected_data_from_norm(): Size of x_g,x_e,x_g_pre,x_e_pre is not the same  '
            return False
        ...
        ## Selected data gonna be there
        x1_g_selected = np.array([])
        x1_e_selected = np.array([])
        x2_g_selected = np.array([])
        x2_e_selected = np.array([])
        y1_g_selected = np.array([])
        y1_e_selected = np.array([])
        y2_g_selected = np.array([])
        y2_e_selected = np.array([])

        ## Save also data wich we discard (if you need to analize it as well)
        x1_g_discarded = np.array([])
        x1_e_discarded = np.array([])
        x2_g_discarded = np.array([])
        x2_e_discarded = np.array([])
        y1_g_discarded = np.array([])
        y1_e_discarded = np.array([])
        y2_g_discarded = np.array([])
        y2_e_discarded = np.array([])
        ...
        ### Data from 1st readout
        ## Working on data with prepared |g>
        for i in range(len(self.x1_g)):
            if g_state(self.x_g_pre[i], self.threshold, self.sign_ge):
                x1_g_selected = np.append(x1_g_selected, self.x1_g[i])
                y1_g_selected = np.append(y1_g_selected, self.y1_g[i])
            else:
                x1_g_discarded = np.append(x1_g_discarded, self.x1_g[i])
                y1_g_discarded = np.append(y1_g_discarded, self.y1_g[i])

        ## Working on data with prepared |e>
        for i in range(len(self.x1_e)):
            if g_state(self.x_e_pre[i], self.threshold, self.sign_ge):
                x1_e_selected = np.append(x1_e_selected, self.x1_e[i])
                y1_e_selected = np.append(y1_e_selected, self.y1_e[i])
            else:
                x1_e_discarded = np.append(x1_e_discarded, self.x1_e[i])
                y1_e_discarded = np.append(y1_e_discarded, self.y1_e[i])

        ### Data from 2nd readout
        ## Working on data with prepared |g>
        for i in range(len(self.x2_g)):
            if g_state(self.x_g_pre[i], self.threshold, self.sign_ge):
                x2_g_selected = np.append(x2_g_selected, self.x2_g[i])
                y2_g_selected = np.append(y2_g_selected, self.y2_g[i])
            else:
                x2_g_discarded = np.append(x2_g_discarded, self.x2_g[i])
                y2_g_discarded = np.append(y2_g_discarded, self.y2_g[i])

        ## Working on data with prepared |e>
        for i in range(len(self.x2_e)):
            if g_state(self.x_e_pre[i], self.threshold, self.sign_ge):
                x2_e_selected = np.append(x2_e_selected, self.x2_e[i])
                y2_e_selected = np.append(y2_e_selected, self.y2_e[i])
            else:
                x2_e_discarded = np.append(x2_e_discarded, self.x2_e[i])
                y2_e_discarded = np.append(y2_e_discarded, self.y2_e[i])

        #####____SAVE_SELECTED_DATA___######
        self.x1_g_select = x1_g_selected
        self.x1_e_select = x1_e_selected
        self.y1_g_select = y1_g_selected
        self.y1_e_select = y1_e_selected

        self.x1_g_discarded = x1_g_discarded
        self.x1_e_discarded = x1_e_discarded
        self.y1_g_discarded = y1_g_discarded
        self.y1_e_discarded = y1_e_discarded

        self.x2_g_select = x2_g_selected
        self.x2_e_select = x2_e_selected
        self.y2_g_select = y2_g_selected
        self.y2_e_select = y2_e_selected

        self.x2_g_discarded = x2_g_discarded
        self.x2_e_discarded = x2_e_discarded
        self.y2_g_discarded = y2_g_discarded
        self.y2_e_discarded = y2_e_discarded

        print 'Selection of readout 1:'
        print '-- before selection --', len(self.x1_g)
        print '-- selected --', len(self.x1_g_select)
        discarded = len(self.x1_g) - len(self.x1_g_select)
        print '-- discarded [%] --', 100.0*( 1.0*discarded / len(self.x1_g) )

        print 'Selection of readout 2:'
        print '-- before selection --', len(self.x2_g)
        print '-- selected --', len(self.x2_g_select)
        discarded = len(self.x2_g) - len(self.x2_g_select)
        print '-- discarded [%] --', 100.0*( 1.0*discarded / len(self.x2_g) )

        self.dict_count_select_ro1 = get_count_states(self.x1_g_select, self.x1_e_select, self.threshold)
        self.dict_count_select_ro2 = get_count_states(self.x2_g_select, self.x2_e_select, self.threshold)
        if self.dict_count_select_ro1 is None or self.dict_count_select_ro2 is None:
            return False
    ...
    # def make_histograms(self, readout_nb=1, nbins=None):
    #     '''
    #     This function in charge of histograms
    #     of fit the gauss and so on
    #     return True or False
    ...
    def save_QNDness_as_object(self, filename, protocol=2):
        '''
        Saving all data using pickle.
        '''
        filename += '.pkl'
    ...
    def autosafe(self, protocol=2):
        try:
            filename = self.datafile[:-4]
            self.save_QNDness_as_object(filename)
            return True
        except:
            print("Cant save in pkl using self.datafile")
            return False
...
class SSResult_pio2(SSResult):

############## WORK on new center of blobs

    def __init__(self, data=None, datafile=None, paramfile=None, param=None, nbins=None, all_included=True, thr_set_zero=True, shift_to_center=True, ext_data=None, ext_threshold=None):
        ...
        if all_included:
            ...
            try:
                ### shift - make histograms symmetrical around zero
                ###
                self.make_norm_data_from_raw(shift=shift_to_center, ext_data=ext_data)
            except:
                print '     ___SSResult error during normalisation'
                self.valid=False
            ...
            ### find the best threshold and shift the data
            try:
                if not ext_threshold:
                    self.set_best_threshold()
                else:
                    self.set_ext_threshold(ext_threshold, ext_data=ext_data)
                if thr_set_zero:
                    self.shift_x_to_threshold_be_zero()
            except:
                print '     ___SSResult error during setting threshold'
            ...
            ### do postselection
            # try:
            if self.make_postselected_data_from_norm(ext_threshold, ext_data=ext_data) != False:
                ### make histograms
                ## flag - True\False if fit was succeed
                flag1 = self.make_histograms(nbins = nbins)
                flag2 = self.make_histograms_y(nbins = nbins)
    ...
    def take_data(self, data):
        ...
        try:
            ...
            ### [re_post, im_post, re, im]
            self.re_g_pre = coef * data[0]  # for now we don't differentiate between g and e states
            self.im_g_pre = coef * data[1]
            self.re_e_pre = coef * data[0]
            self.im_e_pre = coef * data[1]
            self.re_g     = coef * data[2]
            self.im_g     = coef * data[3]
            self.re_e     = coef * data[2]
            self.im_e     = coef * data[3]
            ...
            print 'some error of SSResult_pio2.take_data()'
            return False
    ...
    def load_data(self, datafile):
        ...
        try:
            ...
            raw_data_ss = np.loadtxt(datafile)
            self.re_g_pre = coef * raw_data_ss[:,2]     # data from 1st readout will be used as prepulse
            self.im_g_pre = coef * raw_data_ss[:,3]
            self.re_e_pre = coef * raw_data_ss[:,2]     # for now we don't differentiate between e and g states
            self.im_e_pre = coef * raw_data_ss[:,3]
            self.re_g     = coef * raw_data_ss[:,4]     # data from 2nd readout will be used as results
            self.im_g     = coef * raw_data_ss[:,5]
            self.re_e     = coef * raw_data_ss[:,4]
            self.im_e     = coef * raw_data_ss[:,5]
            print 'data loaded'
            return True
    ...
    def make_norm_data_from_raw(self, shift=True, ext_data=None):
        '''
        Make a normalisation of data on the axis between g_mean and e_mean values
        Write new data as object parameters
        Also make histograms of normalised data
        ...
        if ext_data == None:
            ...
            ext_basis = []
            [ext_c_re_g, ext_c_im_g, ext_c_re_e, ext_c_im_e ] = centers_two_blobs(ext_data[0], ext_data[1], ext_data[2], ext_data[3])
            ext_basis = [np.mean([ext_c_re_g, ext_c_re_e]), np.mean([ext_c_im_g, ext_c_im_e]), np.angle(ext_c_re_e+1j*ext_c_im_e-(ext_c_re_g+1j*ext_c_im_g), deg=True)]
            [threshold_re, threshold_im, theta] = ext_basis       # Load an external basis
        ...
        ### calculated new centers:
        if ext_data == None:
            [c_x_g, c_y_g, c_x_e, c_y_e ] = centers_two_blobs(self.x_g, self.y_g, self.x_e, self.y_e)
        else:
            [c_x_g, c_y_g] = change_basis_point(ext_c_re_g, ext_c_im_g, threshold_re, threshold_im, theta)
            [c_x_e, c_y_e] = change_basis_point(ext_c_re_e, ext_c_im_e, threshold_re, threshold_im, theta)
    ...
    def set_ext_threshold(self, ext_threshold, ext_data=None):
        '''
        Set an external threshold to object
        also calculate set self.dict_count values
        returns True if all good
        '''
        ##########______NORMALIZE IF NECESSARY_____#############################
        if (self.x_g is None) or (self.x_e is None):
            success_norm = self.make_norm_data_from_raw(ext_data=ext_data)
            if not success_norm:
                print 'can not normalise. error of setting threshold'
                return False
    ...
    def make_postselected_data_from_norm(self, ext_threshold=None, ext_data=None):
        '''
        Do the postselection. Threshold usually=0 because we did set_best_threshold_as_zero() before
        also calculate and set dict_count_select
        '''
        if self.threshold is None:
            if ext_threshold != None:
                self.set_best_threshold(ext_threshold=ext_threshold, ext_data=ext_data)
            else:
                success_th = self.set_best_threshold()
                if not success_th:
                    print 'threshold have not been defined yet'
        ...
        if (self.x_g is None) or (self.x_e is None) or (self.x_g_pre is None) or (self.x_e_pre is None):
            ...
            if not ext_threshold:
                ...
                success_th = self.set_ext_threshold(ext_threshold)
            if (not success_norm) or (not success_th):
                print 'can not Postselect. error of normalisation or setting threshold'
                return False
        ...
        ## Selecting which data was e or g state depending on prepulse
        for i in range(len(self.x_g)):
            if g_state(self.x_g_pre[i], self.threshold, self.sign_ge):
                x_g_selected = np.append(x_g_selected, self.x_g[i])
                y_g_selected = np.append(y_g_selected, self.y_g[i])
            else:
                x_e_selected = np.append(x_e_selected, self.x_g[i])
                y_e_selected = np.append(y_e_selected, self.y_g[i])
        ...
        print '-- total shots --', len(self.x_g)
        print '-- selected a g state --', len(self.x_g_select)
        print '-- selected a e state --', len(self.x_e_select)
