...
def measure(vbw_vec,time_sweep_vec, exp, station,current_point=0.):

    I_source.reset()
    I_source.current(current_point*1.e-3)

    meas = qc.Measurement(exp=exp, station=station)
    meas.register_parameter(RS_SA.vbw)
    meas.register_parameter(RS_SA.zero_span_trace,setpoints=[RS_SA.vbw])
    ...
    with meas.run() as datasaver:
        ...
        for idx, f in enumerate(vbw_vec):
            ...
            RS_SA.vbw(f)
            RS_SA.sweep_time(time_sweep_vec[idx])
            ...
            datasaver.add_result((RS_SA.vbw, get_f),
...
SA_vbw_vec    = np.array([3,10,30,0.1e3,0.3e3,1e3,3e3,10e3,30e3,100e3,300e3,1e6,3e6])
SA_vbw_points = len(SA_vbw_vec)
...
SA_time_sweep_vec = 1./(SA_vbw_vec) * SA_npt

# Snapshot parameters
parameter_snap['SA']={'CW_freq':SA_center,'average':SA_avg,
                      'res_bandwidth':SA_rbw,
                      'Npoints':SA_npt,'ref':SA_ref}

print('\ntime estimation for all measurement : {} '
      .format(datetime.timedelta(seconds=sum(SA_time_sweep_vec))))
...
RS_SA.vbw(SA_vbw_vec[0])
RS_SA.rbw(SA_rbw)
RS_SA.avg(SA_avg)
RS_SA.ref_lvl(SA_ref)
RS_SA.npts(SA_npt)
RS_SA.sweep_time(SA_time_sweep_vec[0])
...
measure(SA_vbw_vec,SA_time_sweep_vec,exp=exp, station=station,current_point=current)
